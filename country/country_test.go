...
func TestGetAreaCodeByCode(t *testing.T) {
	...
	values := make(map[string]string, len(countryCodeMapping))
	for key, value := range countryCodeMapping {
		if _, ok := values[value.AreaCode]; ok {
			fmt.Println(value.AreaCode)
			continue
		}
		values[value.AreaCode] = key
